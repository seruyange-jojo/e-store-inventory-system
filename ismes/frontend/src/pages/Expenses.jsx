...
export default function Expenses() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="ledger-card overflow-hidden"><div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Date</th><th>Description</th><th>Category</th><th>Amount</th></tr></thead><tbody>{!loading && expenses.length === 0 && <tr><td colSpan="4" className="empty-cell">No expenses recorded.</td></tr>}{expenses.map((expense) => <tr key={expense.id}><td>{expense.expenseDate}</td><td><strong>{expense.description}</strong></td><td>{expense.categoryName || "Uncategorised"}</td><td>{formatCurrency(expense.amount)}</td></tr>)}</tbody></table></div></div>
    </div>
  );
}